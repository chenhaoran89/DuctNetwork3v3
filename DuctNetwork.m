...
%                     exitflag = 1;
%                 else
%                     linear_interp = @(x,y1,y2) x.*(y2-y1)+y1;
%                     exp_interp = @(x,y1,y2) exp(linear_interp(x,log(y1),log(y2)));
%                     R0_lin = linear_interp(rand(n_tot,1),lb,ub);
%                     R0_exp = exp_interp(rand(n_tot,1),lb,ub);
%                     R0(lb>0)=R0_exp(lb>0);
%                     R0(lb<=0)=R0_lin(lb<=0);
%                 end
            end
        ...
        function [MultiS, cnfg_X, R] = Identification_TRR(obj, cnfg_s_mIdx, proc_ConfigIdx, proc_ob_Measurements, proc_ob_SensorLocation, ob_SensorType, ob_Uncertainty)
            % cnfg_s_mIdx is matrix of size cnfg by s, each of the cell indicates the identified parameter and its index in s_MultiS for each configuration
            % proc_ConfigIdx is vector of size proc by 1, indicating each process uses which configuration.
            % proc_ob_Measurements is matrix of size proc by ob, storing the measured data in each process
            % proc_ob_SensorLocation is matrix of size proc by ob, indicating the position of sensors in each process
            ...
            %             Backup = obj.BackupParameterIdentification();
            obj.Identification_Preprocess(cnfg_s_mIdx, proc_ConfigIdx, proc_ob_Measurements, proc_ob_SensorLocation, ob_SensorType, ob_Uncertainty);
            n_tot = obj.cnfg*obj.t+sum(obj.s_m);
            if verLessThan('matlab','9')
                options = optimoptions(@lsqnonlin,'Display',obj.Display,...
                    'Algorithm','trust-region-reflective',...
                    'TolFun',1e-12,'TolX',1e-12,...
                ...
                options = optimoptions(@lsqnonlin,'Display',obj.Display,...
                    'Algorithm','trust-region-reflective',...
                    'FunctionTolerance',1e-12,'StepTolerance',1e-12,...
                    'MaxIterations',n_tot*25,'OutputFcn',[],...
                    'SpecifyObjectiveGradient',true,'CheckGradients',false,...
                    'FiniteDifferenceType','central','FiniteDifferenceStepSize',1e-10);
            end
            exitflag = -1;
            if isempty(obj.X)
                ...
                R0 = [repmat(obj.X,obj.cnfg,1);cell2mat(arrayfun(@(x,b)repmat(x,b,1),obj.S,obj.s_m,'UniformOutput',false))];
            end
            ub = [repmat(obj.X_ub,obj.cnfg,1);cell2mat(arrayfun(@(x,b)repmat(x,b,1),obj.S_ub,obj.s_m,'UniformOutput',false))];
            lb = [repmat(obj.X_lb,obj.cnfg,1);cell2mat(arrayfun(@(x,b)repmat(x,b,1),obj.S_lb,obj.s_m,'UniformOutput',false))];
            while exitflag<=0
                %                 obj.n_trail = obj.n_trail+1;
                [R,resnorm] = lsqnonlin(@obj.res_Identification,R0,lb,ub,options);
                if resnorm<1e-3
                    exitflag = 1;
                else
                    linear_interp = @(x,y1,y2) x.*(y2-y1)+y1;
            ...
            cnfg_X = num2cell(reshape(R(1:obj.t*obj.cnfg),obj.t,obj.cnfg),1)';
            MultiS = R((obj.cnfg*obj.t+1):end);
            obj.s_MultiS=mat2cell(MultiS,obj.s_m,1);
            %             obj.RecoverParameterIdentification(Backup);
            function stop = Identification_TRR_outfun(x,optimValues,state)
                persistent last_Values last_x N_slow_improvement
                persistent delta_e delta_x
                persistent hist_x hist_residual hist_gradient hist_trustregionradius hist_stepsize
                stop = false;
                switch state
                    case 'init'
                        last_Values = optimValues;
                        last_x = x;
                        N_slow_improvement = 0;
                        delta_e = 0;
                        delta_x = 0;
                        ...
                        hist_residual = [];
                        hist_gradient = [];
                        hist_trustregionradius = [];
                        hist_stepsize = [];
                        
                    case 'iter'
                        delta_e = optimValues.residual-last_Values.residual;
                        delta_x = x - last_x;
                        
                        hist_x = [hist_x,x];
                        hist_residual = [hist_residual, optimValues.residual];
                        hist_gradient = [hist_gradient, optimValues.gradient];
                        hist_trustregionradius = [hist_trustregionradius, optimValues.trustregionradius];
                        hist_stepsize = [hist_stepsize, optimValues.stepsize];
                        
                        if any(x<lb) || any(x>ub)
                            % Algorithm goes out of the range
                            R0 = min(max(x,lb),ub);
                            stop = true;
                            return
                        end
                        ...
                        if optimValues.iteration>5 && norm(x-hist_x(:,end-4))<0.1*sum(hist_stepsize(end-3:end)) && false
                            
                            % Occurs zigzag moves, stop and retry with a new start along the trend
                            x_mean = sum(hist_x(:,end-4:end),2)/5;
                            grad_mean = sum(hist_gradient(:,end-3:end),2)/4;
                            R0 = Move2Improve(x_mean,grad_mean, optimValues.resnorm);
                            % R0 = hist_x(:,end-4:end)*[-2;0;-1;0.5;3.5];
                            % R0 = 3.5*x + 0.5*hist_x(:,end-1) - hist_x(:,end-2) - 2*hist_x(:,end-4);
                            stop = true;
                            return
                        end
                        
                        if norm(delta_e)/norm(optimValues.residual)<1e-6 && false
                            % Descending too slow
                            N_slow_improvement = N_slow_improvement + 1;
                            if N_slow_improvement>10
                                [x_new, alpha] = Move2Improve(x,optimValues);
                                if alpha <= optimValues.trustregionradius
                                    % can not find another point, use a random start point
                                    x_new = [rand(obj.cnfg*obj.t,1);rand(sum(obj.s_m),1)];%.*(ub-lb)+lb;
                                end
                                R0 = x_new;stop = true;
                            end
                        else
                            N_slow_improvement = N_slow_improvement/2;
                        end
                    case 'done'
                        %                         delta_e = optimValues.residual-last_Values.residual;
                        %                         delta_x = x - last_x;
                        if optimValues.iteration>=options.MaxIterations && false
                            % Algorithm exceed maximum iterations
                            if norm(delta_e)/norm(optimValues.residual)<1e-6
                                [x_new, alpha] = Move2Improve(x,optimValues);
                                if alpha <= optimValues.trustregionradius
                                    x_new = [rand(obj.cnfg*obj.t,1);rand(sum(obj.s_m),1)];
                                end
                            else
                                x_new = x;
                            end
                        ...
                        if optimValues.resnorm>options.FunctionTolerance
                            % Algorithm fails to find solution
                            
                            %plot how residual changes with x in any two dimensions
                            for kk = 1:8
                                kk
                                draw_section(kk,9,abs(x(kk))*0.3,abs(x(9))*0.8,20,kk);
                            end
                            
                            %                             if optimValues.trustregionradius<1e-4
                            %                                 %trust region radius is too small, jump to a new point
                            %                                 [x_new, alpha] = Move2Improve(x,optimValues);
                            %                                 if alpha <= optimValues.trustregionradius
                            %                                     % can not find another point, use a random start point
                            %                                     x_new = [rand(obj.cnfg*obj.t,1);rand(sum(obj.s_m),1)];%.*(ub-lb)+lb;
                ...
                function [x_new, alpha] = Move2Boundary(x,gradient)
                    pos_grad_idx=(gradient>0);
                    alpha = min(min((x(pos_grad_idx)-lb(pos_grad_idx))./gradient(pos_grad_idx)),...
                        min(-(ub(~pos_grad_idx)-x(~pos_grad_idx))./gradient(~pos_grad_idx)));
                    x_new = x-gradient*alpha;
                end
                function [x_new, alpha] = Move2Improve(x,gradient, resnorm)
                    [~,alpha] = Move2Boundary(x,gradient);
                    x_new = x-gradient*alpha;
                    while norm(obj.res_Identification(x_new))^2> resnorm
                        alpha = alpha/2;
                        x_new = x-gradient*alpha;
                    end
                end
                function draw_section(xIdx, yIdx, xRange, yRange, MeshDensity, nfig)
                    M = sparse([xIdx,yIdx],[1,2],1,length(x),2);
                    hfun = @(x1,x2)sum(obj.res_Identification(x+M*[x1;x2]).^2);
                    xVec = linspace(-xRange,xRange,MeshDensity+1);
                    yVec = linspace(-yRange,yRange,MeshDensity+1);
                    [xGrid,yGrid] = meshgrid(xVec, yVec);
                    zGrid = arrayfun(hfun,xGrid,yGrid);
                    if nargin>=6
                        figure(nfig);
                        clf
                    else
                    ...
                    mesh(xGrid + x(xIdx), yGrid + x(yIdx), zGrid);
                    drawnow();
                end
        ...
        function [IdentifiedDP, IdentifiedParam] = Identification_CC(obj, MeasureData_Q, MeasureData_P, NonNullIdx, UndetDamperIdx, UndetDamperInitVal, UndetParamIdx, UndetParamInitVal, SensorAccuracy)
            % n_T is the number of terminals/dampers to be identified
            % n_DP is the number of damper positions measured (including fully open)
            % n_M is the equivalent iterations of measurements, i.e., the number of simulations, n_M = (n_DP-1)*n_T+1
            
            % MeasureData_Q / MeasureData_P is an n_M by n_T matrix
            % MeasureData_Q     MeasureData_P       NonNullIdx      PositionIdx
            % q q q             p p p               1 1 1           0 0 0
            % q - -             p - -               1 0 0           1 0 0
            % q - -             p - -               1 0 0           2 0 0
            % - q -             - p -               0 1 0           0 1 0
            % - q -             - p -               0 1 0           0 2 0
            % - - q             - - p               0 0 1           0 0 1
            % - - q             - - p               0 0 1           0 0 2
            % NonNullIdx is the corresponding logical matrix, where 0 means invalid/null element
            % A = 1:n_M:1+(n_T-1)*n_M; B = repmat(A,[n_DP-1,1]); Idx = sort([A,(1:n_M-1)+B(1:end)]);
            % NonNullIdx = false(n_M,n_T); NonNullIdx(Idx) = true;
            % PositionIdx records the damper positions for each damper, where 0-2 represents 3 levels
            % PositionIdx = zeros(n_M,n_T); PositionIdx(NonNullIdx) = repmat(0:n_DP-1,[1,n_T]);
            
            % UndetDamperIdx is an n_T column vector, which contains the index of the undetermined dampers
            % UndetParamIdx is an p column vector, which contains the index of the undetermined parametres (ex. duct length) in S list
            % IdentifiedDP is a n_DP by n_T matrix, which gives the identified resistance/position at all 3 levels for each damper
            % IdentifiedParam and UndetParamIdx have the same shape
            % SensorAccuracy = [Q_accuracy, P_accuracy]
            
            % Variables should be prepared in measurement.m:
            % MeasureData_Q: Q(obj.gdr_Bidx(UndetDamperIdx))
            % MeasureData_P: P(obj.gdr_Nidx(UndetDamperIdx))
            % NonNullIdx
            
            [n_M,n_T] = size(NonNullIdx);
            n_DP = (n_M -1)/n_T+1;
            PositionIdx = zeros(n_M,n_T);
            PositionIdx(NonNullIdx) = repmat(0:n_DP-1,[1,n_T]);
            f = @(x)obj.Res_Identification_CC(x,n_T, n_DP, n_M, MeasureData_Q, MeasureData_P, NonNullIdx, PositionIdx, UndetDamperIdx, UndetParamIdx, SensorAccuracy);
            x0 = [UndetDamperInitVal(:);UndetParamInitVal(:)];
            %x0 = rand(n_T*n_DP+length(UndetParamIdx),1);
            options = optimoptions('lsqnonlin','SpecifyObjectiveGradient',true,...
                'CheckGradients',false,'FiniteDifferenceType','central',... % derivative check fails, reason unkown
                'Display','iter',...
                'FunctionTolerance',1e-12,...
                'StepTolerance',1e-8,...
                'OptimalityTolerance',1e-12,...
                'MaxIterations',400);
%                 'PlotFcn',{@optimplotx,@optimplotfval,@optimplotresnorm,@optimplotfirstorderopt},...
            X = lsqnonlin(f,x0,zeros(size(x0)),[],options); % lower bound: use zeros(size(x0)) rather than 0
            IdentifiedDP = reshape(X(1:n_DP*n_T),[n_DP,n_T]);
            IdentifiedParam = X(n_DP*n_T+1:end);
        end
        
        function [residual, Jacobian] = Res_Identification_CC(obj, X, n_T, n_DP, n_M, MeasureData_Q, MeasureData_P, NonNullIdx, PositionIdx, UndetDamperIdx, UndetParamIdx, SensorAccuracy)
            % x is a column vector, x = [IdentifiedDP(:);IdentifiedParam], length(x) = n_T*n_DP + length(UndetParamIdx)
            
            PredictData_Q = MeasureData_Q;
            PredictData_P = MeasureData_P;
            
            IdentifiedDP = reshape(X(1:n_DP*n_T),[n_DP,n_T]);
            IdentifiedParam = X(n_DP*n_T+1:end); % column vector
            S_P = obj.S;
            S_P(UndetParamIdx) = IdentifiedParam;
            PositionList = repmat(IdentifiedDP(1,:),[n_M,1]); % instead of creating zeros matrix
            PositionList(NonNullIdx) = IdentifiedDP(:); % creat a PositionList which has the same shape with NonNullIdx
            
            Jacobian_Q_DP = zeros(n_DP*n_T, n_DP*n_T);
            Jacobian_P_DP = Jacobian_Q_DP;
            Jacobian_Q_Param = zeros(n_DP*n_T, length(IdentifiedParam));
            Jacobian_P_Param = Jacobian_Q_Param;
            
            for i = 1:n_M
                Position = PositionList(i,:);
                S_P(obj.d_Sidx(UndetDamperIdx)) = Position;
                [X_internal, Q, P] = obj.Sim(S_P); % wrong function name, use Sim instead of sim
                [~,~,~,~,ddPdX,ddPdS,dXdS] = obj.res_Sim(X_internal, S_P);
                dQdS = obj.U*dXdS;
                dPdS = (obj.A*obj.A')\obj.A*(ddPdX*dXdS+ddPdS);
                PredictData_Q(i,:) = Q(obj.gdr_Bidx(UndetDamperIdx));
                PredictData_P(i,:) = P(obj.gdr_Nidx(UndetDamperIdx));
                if i == 1
                    row_select = 1:n_DP:n_DP*n_T;
                    t = 1:n_T;
                else
                    t = ceil((i-1)/(n_DP-1));
                    row_select = i+t-1;
                end
                Jacobian_Q_DP(row_select,PositionIdx(i,:)+(1:n_DP:n_DP*n_T)) = dQdS(obj.gdr_Bidx(UndetDamperIdx(t)),obj.d_Sidx(UndetDamperIdx));
                Jacobian_P_DP(row_select,PositionIdx(i,:)+(1:n_DP:n_DP*n_T)) = dPdS(obj.gdr_Nidx(UndetDamperIdx(t)),obj.d_Sidx(UndetDamperIdx));
                Jacobian_Q_Param(row_select,:) = dQdS(obj.gdr_Bidx(UndetDamperIdx(t)),UndetParamIdx); % use UndetParamIdx instead of obj.d_Sidx(UndetParamIdx)
                Jacobian_P_Param(row_select,:) = dPdS(obj.gdr_Nidx(UndetDamperIdx(t)),UndetParamIdx);
            end
            residual_Q = (PredictData_Q(NonNullIdx)-MeasureData_Q(NonNullIdx))/SensorAccuracy(1); % column vector
            residual_P = (PredictData_P(NonNullIdx)-MeasureData_P(NonNullIdx))/SensorAccuracy(2); % column vector
            residual = [residual_Q;residual_P];
            Jacobian = [Jacobian_Q_DP/SensorAccuracy(1),Jacobian_Q_Param/SensorAccuracy(1);Jacobian_P_DP/SensorAccuracy(2),Jacobian_P_Param/SensorAccuracy(2)];
        end
    end
...
%                         [dP, dPdQ, dPdS] = DuctNetwork.Fan(q,s,DuctNetwork.Table_NVT250.LnP_Coeff,DuctNetwork.Table_NVT250.LnQ_Coeff,DuctNetwork.Table_NVT250.Eff_max);
    ...
    methods (Static = true)
        ...
        function [xCurrent, resnorm, EXITFLAG] = lsq_adam(FUN,xCurrent)
            stepSize = 0.001;
            beta1 = 0.9;
            beta2 = 0.999;
            epsilon = 1e-8;
            ...
            nDecVar = length(xCurrent);
            
            m = zeros(nDecVar, 1);
            v = zeros(nDecVar, 1);
            
            disp('Iteration  f(x)   Norm of step  1st order optimality')
            for i = 1:nIter
                ...
                step = - stepSize.*mHat./(sqrt(vHat) + epsilon);
                xCurrent = xCurrent + step;
                fprintf('%6d   %10g  %10g  %10g\n',i, 0.5*y'*y, norm(step), norm(GRAD))
            end
            resnorm = 0.5*y'*y;
            EXITFLAG = 1;
        end
        
        function [xCurrent, resnorm, EXITFLAG] = lsq_gd(FUN,xCurrent)
            stepSize = 0.0001;
            nIter = 100;
